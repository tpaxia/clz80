...
;
;
; SEARLE's ORIGINAL BASIC WORK SPACE LOCATIONS
;
; 0200H - 2000H	BASIC EXECUTABLE
; 2000H - 2045H SERIAL INTERFACE VARIABLES
; 2045H - 213DH BASIC EXECUTABLE VARIABLES / WORKSPACE
; 213EH - BASIC PROGRAM START

;==================================================================================
;
; NANOCOMPUTER CLZ80 BASIC WORK SPACE LOCATIONS
;
; E000H - FFFFH	BASIC EXECUTABLE
;   00H -  FFH JUMP TABLES
;  100H - 145H SERIAL INTERFACE VARIABLES
;  145H - 23DH BASIC EXECUTABLE VARIABLES / WORKSPACE
;  23EH -      BASIC PROGRAM START
;
;==================================================================================
...
WRKSPC  .EQU    145H             ; BASIC Work space


; End of configuration changes to run on CLZ80. 
; Startup and Serial routines are at the end of the file
;==================================================================================
...
INIT2:   LD      DE,INITAB       ; Initialise workspace
...
;------------------------------------------------------------------------------
;  SGS ATES NANOCOMPUTER CLZ80 8K BASIC ROM CODE
;------------------------------------------------------------------------------

TXEmpty         .EQU      0
RXEmpty         .EQU 	  1 
TermStatus      .EQU 	  01H            ; 8251 Status
TermData    .equ 00H                     ; 8251 Data

RTS_HIGH        .EQU     07H            ; DTR=0 RTS=1
RTS_LOW         .EQU     27H            ; DTR=0 RTS=0

SER_BUFSIZE     .EQU     3FH
SER_FULLSIZE    .EQU     30H
SER_EMPTYSIZE   .EQU     5
 
serBuf          .EQU     $100            ; For memory map see beginning of the file
serInPtr        .EQU     serBuf+SER_BUFSIZE
serRdPtr        .EQU     serInPtr+2
serBufUsed      .EQU     serRdPtr+2
basicStarted    .EQU     serBufUsed+1
TEMPSTACK       .EQU     CURPOS-3        ; Was $20ED Top of BASIC line input 
                                         ; buffer so is "free ram" when BASIC resets

;------------------------------------------------------------------------------
;
;  INIT ROUTINE FOR NBZ80
; 
;------------------------------------------------------------------------------
...
INIT:
			   ...
			   DI
			   ; First configure PIO in input mode
			   ...
			   LD A,03H ; INTERRUPT CONTROL
				
			   OUT (6),A  ; PORT A
			   
			   LD A,40H ; INTERRUPT VECTOR
			   OUT (7),A  ; PORT B
				
			   
			   LD A,87H ; INTERRUPT CONTROL
			   OUT (7),A  ; PORT B
				
               LD DE,0                 ; COPY RST VECTORS TO PAGE 0				
			   LD HL,RST00
			   LD BC,serialIntEnd-RST00 ; Length of Page 0 + interrupt routine
		       LDIR 
               ...
               LD        (serBufUsed),A
										 ; Initialize 8251
               XOR       A
			   LD        I,A             ; Interrupt vectors on page 0
               OUT       (TermStatus),A  ; Put the USART in known state
			   OUT       (TermStatus),A  ; 
			   OUT       (TermStatus),A  ; 
			   
			   LD        A,40H
			   OUT       (TermStatus),A  ; RESET BIT
			   
			   LD        A,0CEH
			   OUT       (TermStatus),A  ; 2 bits no parity 16x. Baud rate set by jumpers
			   LD        A,17H
			   OUT       (TermStatus),A  ; Enable RX & TX & RESET ERRORS. Set DTR to 0
			   		  
			   ; Setup interrupt mask and interrupt mode
			   LD      A,$FF
			   LD      I,A
			   IM      1
...
SIGNON1:       .BYTE     CS
               .BYTE     "CLZ80 Basic (i8251)",CR,LF,0
SIGNON2:       .BYTE     CR,LF
...
conout1:        IN       A,(TermStatus)  ; Status byte       
                BIT      TXEmpty,A       ; Set Zero flag if still transmitting character       
                JR       Z,conout1       ; Loop until flag signals ready
                POP      AF              ; Retrieve character
                OUT      (TermData),A         ; Output the character
                RET
...
_PRINT:          LD       A,(HL)          ; Get character
                OR       A               ; Is it $00 ?
                RET      Z               ; Then RETurn on terminator
                RST      08H             ; Print it
                INC      HL              ; Next Character
                JR       _PRINT           ; Continue until $00
                RET
;------------------------------------------------------------------------------
;
;  Moved here from basic code above to keep the rest under FC02
; 
;------------------------------------------------------------------------------
...
serialInt:      PUSH     AF
                ...
                IN       A,(TermStatus)
                BIT      RXEmpty,A       ; Check if interupt due to read buffer full
